...
function showSuccessToast(message) {
    ...
    const toast = new bootstrap.Toast(toastElement, { delay: 1000 }); // 1 seconds
    toast.show();
}
function showFailedToast(message) {
    ...
    const toast = new bootstrap.Toast(toastElement, { delay: 1000 }); // 3 seconds
    toast.show();
}
...
async function bulkPrintCardsPDF(memberIds, cardType) {
    if (!memberIds || memberIds.length === 0) {
        alert("No members selected to print.");
        return;
    }
    ...
    showLoading();
    try {
        const response = await fetch(`${apiBaseUrl}/MemberImages/generate-cards-pdf`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
...
async function bulkPrintCardsPrint(memberIds, cardType) {
    if (!memberIds || memberIds.length === 0) {
        alert("No members selected to print.");
        return;
    }
    ...
    showLoading();
    try {
        const response = await fetch(`${apiBaseUrl}/MemberImages/generate-cards-print`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
