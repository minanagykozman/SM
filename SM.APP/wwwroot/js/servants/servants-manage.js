...
function openCreateModal() {
    ...
    $('#servantModalTitle').text('Create Servant');
    $('#btnSaveServant').text('Create Servant');
    servantModal.show();
}
// ============================================
// EDIT MODAL
...
async function openEditModal(servantID) {
    try {
        ...
        $('#Email').val(servant.email).prop('disabled', true);
        $('#Mobile').val(servant.mobile1);
        $('#Mobile2').val(servant.mobile2);
        $('#isActive').prop('checked', servant.isActive);
        $('.password-field').hide();
        if (servant.servantClassesIDs) {
            $('#drpServantClasses').val(servant.servantClassesIDs).trigger('change');
        }
...
async function saveServant() {
    ...
    try {
        showLoading();
        const url =servantID === 0? `${apiBaseUrl}/Servants/Register`: `${apiBaseUrl}/Servants/Update`;
        ...
        showSuccessToast(servantID === 0 ? "Servant created successfully" : "Servant updated successfully");
        await loadServants();
    }
    catch (err) {
        console.error(err);
